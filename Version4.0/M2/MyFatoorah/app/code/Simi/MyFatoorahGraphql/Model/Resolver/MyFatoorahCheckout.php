...
class MyFatoorahCheckout implements ResolverInterface
{
    ...
    {
        if (!isset($args['cartId']) || !isset($args['addressArray']) || !isset($args['gateway'])||
            empty($args['cartId']) || empty($args['addressArray']) || empty($args['gateway']))
        {
            throw new GraphQlInputException(__('Invalid parameter list.'));
        }
        $redirectUrl = $this->checkout($args['cartId'], $args['addressArray'], $args['gateway']);
        $output = [];
        $output['urlRedirect'] = $redirectUrl;
        return $output ;
    }

    public function checkout($cartId, $addressArray, $gateway = null) {

...
//---------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    private function savePaymentData($data) {
        ...
        $cards              = $this->_gatewayConfig->getVendorGateways();
        $gatewayName        = $itemData['gateway_name'];

        if (empty($itemData['payment_id']) || $itemData['payment_id'] != $transaction->PaymentId) {
            foreach ($cards as $card) {
                if ($card->getText() == $transaction->PaymentGateway) {
                    $gatewayName = $card->getText();
                }
            }
            $item->setData('gateway_name', $gatewayName);
...
//            if ($isProductSale) {
                $connection = $this->getObjectManager()->create('Magento\Framework\App\ResourceConnection')->getConnection();
                $table      = $connection->getTableName('inventory_reservation');

		foreach ($order->getAllItems() as $item) {
                    $sku         = $item->getSku();
                    $selectQuery = "select * from " . $table . " where sku = '" . $sku
                    ...
                    if ($result && sizeof($result) > 0) {
                        $deleteQuery  = "delete from " . $table . " where sku = '" . $sku
                                . "' and metadata like '{\"event_type\":\"order_canceled\",\"object_type\":\"order\",\"object_id\":\"" . $order->getId() . "\"%'";
